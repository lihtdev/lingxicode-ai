package com.lihtdev.codesense.i18n
 ...
 * CodeSense AI 国际化资源束。
 *
 * 资源文件位于 src/main/resources/messages/CodeSenseBundle.properties（中文默认）
 * 与 CodeSenseBundle_en.properties（英文）。
 *
 * 用法：CodeSenseBundle.message("key") 或 CodeSenseBundle.message("key", param1, param2)
 *
 * 在 IntelliJ 平台运行时通过 DynamicBundle 加载；在单测环境（无 ApplicationManager）
 * 则回退到 Java 标准 ResourceBundle 从 classpath 加载。
 */
class CodeSenseBundle : DynamicBundle(BUNDLE_PATH) {

    companion object {
        @NonNls
        private const val BUNDLE_PATH = "messages.CodeSenseBundle"

        private var instance: CodeSenseBundle? = null
        ...
        private fun getInstance(): CodeSenseBundle {
            if (instance == null) {
                instance = com.intellij.openapi.application.ApplicationManager.getApplication()
                    .getService(CodeSenseBundle::class.java)
            }
            return instance!!
        }
