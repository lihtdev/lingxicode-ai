...
import com.lihtdev.lingxicode.ui.ChipComponents
...
class SettingsConfigurable : Configurable {
    ...
    init {
        ...
        mainPanel = FormBuilder.createFormBuilder()
            ...
            .addSeparator()
            .addLabeledComponent(JLabel(LingxiCodeBundle.message("settings.outputLanguage")), outputLanguageCombo)
            .addLabeledComponent(JLabel(LingxiCodeBundle.message("settings.uiLanguage")), uiLanguageCombo)
            .addLabeledComponent(JLabel(LingxiCodeBundle.message("settings.maxDiff")), maxDiffField)
    ...
    private fun createHeaderPanel(): JPanel {
        ...
        val authorLabel = JLabel(LingxiCodeBundle.message("settings.header.author")).apply {
            font = JBFont.small()
            foreground = UIUtil.getContextHelpForeground()
        }
    ...
    private fun createTablePanel(): JPanel {
        val titleLabel = JLabel(LingxiCodeBundle.message("settings.providerList.title")).apply {
            font = JBFont.label().asBold()
        }
        val addButton = JButton(LingxiCodeBundle.message("settings.addProvider")).apply {
    ...
    override fun getDisplayName(): String = LingxiCodeBundle.message("settings.displayName")
    ...
    private fun confirmRemoveProvider(config: AiProviderConfig): Boolean {
        ...
        val result = Messages.showYesNoDialog(
            null as Project?,
            LingxiCodeBundle.message("settings.deleteProvider.confirm", modelName),
            LingxiCodeBundle.message("settings.deleteProvider.confirm.title"),
            LingxiCodeBundle.message("settings.deleteProvider.confirm.ok"),
            LingxiCodeBundle.message("settings.deleteProvider.confirm.cancel"),
    ...
    private class AddProviderDialog(
        ...
        private val testButton = JButton(LingxiCodeBundle.message("settings.testConnection")).apply {
        ...
        private val selectAllButton = JButton(LingxiCodeBundle.message("settings.addProvider.selectAll")).apply {
        ...
        init {
            title = LingxiCodeBundle.message("settings.addProvider.title")
            setOKButtonText(LingxiCodeBundle.message("settings.addProvider.confirm"))
            setCancelButtonText(LingxiCodeBundle.message("settings.addProvider.cancel"))
        ...
        private fun iconButton(icon: Icon, tipKey: String?, action: () -> Unit): JButton =
            JButton(icon).apply {
                ...
                cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
                tipKey?.let { toolTipText = LingxiCodeBundle.message(it) }
                addActionListener { action() }
            }

        override fun createCenterPanel(): JComponent {
            val hintLabel = JBLabel(LingxiCodeBundle.message("settings.addProvider.hint")).apply {
                foreground = UIUtil.getContextHelpForeground()
                font = JBFont.small()
            ...
            // 模型列表操作行 + 手动添加行
            val fetchButton = JButton(LingxiCodeBundle.message("settings.addProvider.fetchModels")).apply {
            ...
            val manualRow = JPanel(FlowLayout(FlowLayout.LEFT, 4, 0)).apply {
                ...
                add(JButton(LingxiCodeBundle.message("settings.addProvider.manualAdd")).apply {
            ...
            val providerLabel = JLabel(LingxiCodeBundle.message("settings.provider")).apply {
                minimumSize = preferredSize
            }
            val planTypeLabel = JLabel(LingxiCodeBundle.message("settings.planType")).apply {
                minimumSize = preferredSize
            }
            val baseUrlLabel = JLabel(LingxiCodeBundle.message("settings.baseUrl")).apply {
                minimumSize = preferredSize
            }
            val apiKeyLabel = JLabel(LingxiCodeBundle.message("settings.apiKey")).apply {
                minimumSize = preferredSize
            }

            // 区块标题：加粗灰色小字，把对话框分成「提供商信息 / 选择模型」两区
            val providerSectionCaption = JBLabel(LingxiCodeBundle.message("settings.addProvider.section.provider")).apply {
                font = JBFont.label().asBold()
                foreground = UIUtil.getContextHelpForeground()
            }
            val modelsSectionCaption = JBLabel(LingxiCodeBundle.message("settings.addProvider.section.models")).apply {
                font = JBFont.label().asBold()
                foreground = UIUtil.getContextHelpForeground()
            }
        ...
        private fun refreshProviderInfo(option: ProviderComboOption, providerChanged: Boolean) {
            ...
            deleteProviderButton.toolTipText = LingxiCodeBundle.message(
        ...
        private fun testConnection() {
            ...
            if (baseUrl.isEmpty()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.test.warn.config"),
                    LingxiCodeBundle.message("settings.addProvider.title"),
                )
                return
            }
            ...
            if (key.isBlank()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.test.warn.key"),
                    LingxiCodeBundle.message("settings.addProvider.title"),
                )
                return
            }
            ...
            val taskTitle = LingxiCodeBundle.message("settings.test.progress", provider.displayName)
                ...
                override fun run(indicator: ProgressIndicator) {
                    val reply = try {
                        ...
                        ApplicationManager.getApplication().invokeLater {
                            Messages.showErrorDialog(
                                contentPanel,
                                LingxiCodeBundle.message("notification.testFail", e.message ?: ""),
                                LingxiCodeBundle.message("settings.addProvider.title"),
                            )
                        }
                        return
                    ...
                    ApplicationManager.getApplication().invokeLater {
                        Messages.showInfoMessage(
                            contentPanel,
                            LingxiCodeBundle.message("notification.testSuccess", elapsed.toString(), reply.take(50)),
                            LingxiCodeBundle.message("settings.addProvider.title"),
                        )
                    }
                }
        ...
        private fun fetchModels() {
            ...
            val taskTitle = LingxiCodeBundle.message("settings.addProvider.fetchingModels")
            object : Task.Backgroundable(null, taskTitle, true) {
                override fun run(indicator: ProgressIndicator) {
                    val models = try {
                        ...
                        ApplicationManager.getApplication().invokeLater {
                            Messages.showWarningDialog(
                                contentPanel,
                                LingxiCodeBundle.message("settings.addProvider.fetchFail", e.message ?: ""),
                                LingxiCodeBundle.message("settings.addProvider.title"),
                            )
                        }
                        return
                    ...
                    ApplicationManager.getApplication().invokeLater {
                        ...
                        Messages.showInfoMessage(
                            contentPanel,
                            LingxiCodeBundle.message("settings.addProvider.fetchSuccess", models.size.toString()),
                            LingxiCodeBundle.message("settings.addProvider.title"),
                        )
                    }
                }
        ...
        override fun doOKAction() {
            ...
            if (baseUrl.isEmpty()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.baseUrl.empty"),
                    LingxiCodeBundle.message("settings.addProvider.title"),
                )
                return
            }
    ...
    private class ProviderEditorDialog(
        ...
        private val dialogTitle = LingxiCodeBundle.message(
            if (existing == null) {
        ...
        override fun createCenterPanel(): JComponent {
            val baseUrlWithNote = JPanel(BorderLayout()).apply {
                ...
                add(JBLabel(LingxiCodeBundle.message("settings.addProvider.protocolNote")).apply {
                    font = JBFont.small()
                    foreground = UIUtil.getContextHelpForeground()
        ...
        override fun doOKAction() {
            ...
            if (name.isEmpty()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.addProvider.name.empty"),
                    dialogTitle,
                )
                return
            }
    ...
    private class ModelTagsPanel : JPanel(FlowLayout(FlowLayout.LEFT, 4, 2)) {
        ...
        private val inlineInput = JTextField(10).apply {
            toolTipText = LingxiCodeBundle.message("settings.editProvider.tags.placeholder")
        }
        private val confirmButton = JButton(AllIcons.Actions.Commit).apply {
            toolTipText = LingxiCodeBundle.message("settings.addProvider.confirm")
            isFocusable = false
            margin = JBUI.insets(2)
            addActionListener { commitInlineInput() }
        }
        private val cancelButton = JButton(AllIcons.Actions.Close).apply {
            toolTipText = LingxiCodeBundle.message("settings.addProvider.cancel")
            isFocusable = false
            margin = JBUI.insets(2)
        ...
        private val addButton = JButton(AllIcons.General.Add).apply {
            toolTipText = LingxiCodeBundle.message("settings.editProvider.tags.add")
            isFocusable = false
            margin = JBUI.insets(2)
        ...
        private fun createChip(text: String, index: Int): JComponent {
            ...
            val removeButton = JButton(AllIcons.Actions.Close).apply {
                ...
                toolTipText = LingxiCodeBundle.message("settings.action.delete")
                addActionListener {
    ...
    private class EditProviderDialog(
        ...
        private val fetchModelButton = JButton(LingxiCodeBundle.message("settings.editProvider.fetchModels")).apply {
            addActionListener { fetchModels() }
        ...
        init {
            title = LingxiCodeBundle.message("settings.editProvider.title")
            setOKButtonText(LingxiCodeBundle.message("settings.addProvider.confirm"))
            setCancelButtonText(LingxiCodeBundle.message("settings.addProvider.cancel"))
            loadInitial()
            init()
        ...
        override fun createCenterPanel(): JComponent {
            ...
            // 标签提示（最多 N 个）
            val tagsHintLabel = JBLabel(LingxiCodeBundle.message("settings.editProvider.tags.max", ModelTagsPanel.MAX_TAGS.toString())).apply {
                font = JBFont.small()
                foreground = UIUtil.getContextHelpForeground()
                border = JBUI.Borders.empty(2, 0, 0, 0)
            }
            ...
            val tagsLabel = JLabel(LingxiCodeBundle.message("settings.editProvider.tags")).apply {
                minimumSize = preferredSize
            }
            val providerLabel = JLabel(LingxiCodeBundle.message("settings.provider")).apply {
                minimumSize = preferredSize
            ...
            val providerSectionCaption = JBLabel(LingxiCodeBundle.message("settings.editProvider.section.provider")).apply {
                font = JBFont.label().asBold()
                foreground = UIUtil.getContextHelpForeground()
            }
            val modelSectionCaption = JBLabel(LingxiCodeBundle.message("settings.editProvider.section.model")).apply {
                font = JBFont.label().asBold()
                foreground = UIUtil.getContextHelpForeground()
            }
        ...
        private fun testConnection() {
            ...
            if (baseUrl.isEmpty() || model.isEmpty()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.test.warn.config"),
                    LingxiCodeBundle.message("settings.editProvider.title"),
                )
                return
            }
            val key = String(apiKeyField.password)
        ...
        private fun fetchModels() {
            ...
            val taskTitle = LingxiCodeBundle.message("settings.editProvider.fetchingModels")
            object : Task.Backgroundable(null, taskTitle, true) {
                override fun run(indicator: ProgressIndicator) {
                    val models = try {
                        OpenAiCompatClient().listModels(provider, key)
                        ...
                        ApplicationManager.getApplication().invokeLater {
                            Messages.showWarningDialog(
                                contentPanel,
                                LingxiCodeBundle.message("settings.editProvider.fetchFail", e.message ?: ""),
                                LingxiCodeBundle.message("settings.editProvider.title"),
                            )
                        }
                        return
                    ...
                    ApplicationManager.getApplication().invokeLater {
                        ...
                        Messages.showInfoMessage(
                            contentPanel,
                            LingxiCodeBundle.message("settings.editProvider.fetchSuccess", models.size.toString()),
                            LingxiCodeBundle.message("settings.editProvider.title"),
                        )
                    }
                }
        ...
        override fun doOKAction() {
            ...
            if (model.isEmpty()) {
                Messages.showWarningDialog(
                    contentPanel,
                    LingxiCodeBundle.message("settings.editProvider.modelEmpty"),
                    LingxiCodeBundle.message("settings.editProvider.title"),
                )
                return
            }
    ...
    private class ModelTableModel : AbstractTableModel() {
        ...
        override fun getColumnName(column: Int): String = when (column) {
            0 -> LingxiCodeBundle.message("settings.table.model")
            1 -> LingxiCodeBundle.message("settings.table.tags")
            2 -> LingxiCodeBundle.message("settings.table.provider")
            3 -> LingxiCodeBundle.message("settings.table.type")
            4 -> LingxiCodeBundle.message("settings.table.actions")
            else -> ""
        }
    ...
    private class ModelTable(
        ...
        override fun getToolTipText(e: MouseEvent): String? {
            ...
            if (col == ACTION_COLUMN) {
                ...
                val p = java.awt.Point(e.x - cellRect.x, e.y - cellRect.y)
                return when {
                    actionRenderer.editBounds.contains(p) -> LingxiCodeBundle.message("settings.action.edit")
                    actionRenderer.deleteBounds.contains(p) -> LingxiCodeBundle.message("settings.action.delete")
                    else -> null
                }
            }
    ...
    companion object {
        ...
        fun requiredLabel(key: String): String =
            "<html>${LingxiCodeBundle.message(key)}<font color='red'>*</font></html>"
    }
}