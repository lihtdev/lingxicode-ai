...
import com.lihtdev.lingxicode.settings.SettingsConfigurable
import com.lihtdev.lingxicode.settings.providerPlanTypeLabel
...
object ModelSwitcherPopup {
    ...
    fun showFirstLevel(project: Project, owner: Component, onChanged: () -> Unit) {
        ...
        if (provider != null) {
            ...
            val noModelLabel = JBLabel(LingxiCodeBundle.message("popup.currentModel.noModel")).apply {
                font = JBFont.small()
                foreground = UIUtil.getContextHelpForeground()
            }
        ...
        val switchButton = JButton(LingxiCodeBundle.message("popup.switchAction")).apply {
            addActionListener {
                popupRef.closeOk(null)
    ...
    private class GroupedEntryRenderer(
        private val activeProvider: com.lihtdev.lingxicode.settings.AiProviderConfig?,
    ...
    private fun createBrandBar(onGearClick: (() -> Unit)?): JComponent {
        val logo = JLabel(IconLoader.getIcon("/icons/lingxicode.svg", ModelSwitcherPopup::class.java))
        val nameLabel = JBLabel(LingxiCodeBundle.message("popup.brand.name")).apply {
        ...
        if (onGearClick != null) {
            ...
            bar.add(JButton(AllIcons.General.Settings).apply {
                ...
                toolTipText = LingxiCodeBundle.message("popup.settings.tooltip")
    ...
    private fun sectionHeader(key: String): JComponent = JBLabel(LingxiCodeBundle.message(key)).apply {
        font = JBFont.label().asBold()
