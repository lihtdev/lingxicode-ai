 ...
 * 编辑器右键「逐行注释」入口。
 *
 * 优先为选中的代码块逐行生成注释，否则处理光标所在的类 / 方法 / 函数；
 * 采集失败（无有效目标）时由 [CommentCodeStarter] 给出警告通知，不发起请求。
 *
...
class CommentCodeAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        CommentCodeStarter.trigger(project, CodeContextBuilder.build(e))
    }
    ...
    override fun update(e: AnActionEvent) {
        ...
        e.presentation.setText(LingxiCodeBundle.message("action.commentCode.text"))
        e.presentation.setDescription(LingxiCodeBundle.message("action.commentCode.description"))
    }
