...
LingxiCode AI（灵犀码）是面向 JetBrains IDE 的 AI 能力集插件（插件 id `com.lihtdev.lingxicode`）。v1 功能：在 Git 提交对话框一键调用大模型，根据勾选的代码变更生成 Conventional Commits 格式提交信息并回填提交框。目标平台 2024.2+（sinceBuild 242），仅依赖 `com.intellij.modules.platform`，全系列 IDE 可用。深入设计细节（组件职责表、数据流、错误处理）见 `docs/design.md`。
...
# 单个测试类 / 单个测试方法
./gradlew test --tests "com.lihtdev.lingxicode.ai.ResponseCleanerTest"
./gradlew test --tests "com.lihtdev.lingxicode.ai.ResponseCleanerTest.方法名"
...
```
action（GenerateCommitMessageAction 提交框按钮 / LingxiCodeActionGroup 编辑器右键预留组）
...
- **扩展机制（新增 AI 功能的唯一路径）**：实现 `feature/AiFeature`（`buildPrompt` 在后台线程执行、`handleResult` 在 EDT 执行），经 plugin.xml 的 `lingxicode.aiFeature` 扩展点注册，并挂载到 `LingxiCode.EditorGroup`。执行层（AiInvocationService）与 AI 层（OpenAiCompatClient）不需改动。
...
- **密钥与配置**：API Key 存 IntelliJ PasswordSafe（serviceName=LingxiCodeAI，key=providerId），不落盘；其余设置经 `PersistentStateComponent` 持久化（`lingxicode-ai.xml`）。
